...
%       IN: dx = task velocity (primary goal)
%       IN: dqCS = configuration space velocity (secondary goal)
%       IN: J = task jacobian
%       OUT: dq = joint velocity (solution)
%       OUT: s = task scale
%       OUT: sCS = configuration space (CS) task scale
...
for iTest = 1:nTest
    ...
    dqCSGoal = dqLow + (dqUpp - dqLow) .* rand(nJnt, 1);

    % solve the problem:
    testPass = true;
    startTime = tic;
    [dq, s, sCS, exitCode] = solver(dqLow, dqUpp, dxGoal, dqCSGoal, J);
