function result = runTest_snsIk_acc_cs(solver, nTest, optTol, cstTol, fid)
...
%       IN: ddx = task velocity (primary goal)
%       IN: ddqCS = configuration space (CS) task acceleration (secondary goal)
%       IN: J = task jacobian
%       IN: dJddq = task jacobian rate * joint angle rate
%       OUT: ddq = joint velocity (solution)
%       OUT: s = task scale
%       OUT: sCS = nullspace configuration space (CS) task scale
...
for iTest = 1:nTest
    ...
    % compute nullspace CS task velocity, then scale
    ddqCSGoal = ddqLow + (ddqUpp - ddqLow) .* rand(nJnt, 1);

    % solve the problem:
    testPass = true;
    startTime = tic;
    [ddq, s, sCS, exitCode] = solver(ddqLow, ddqUpp, ddxGoal, ddqCSGoal, J, dJdq);
