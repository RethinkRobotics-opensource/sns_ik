  ...
  *jointVelocity = VectorD::Zero(robotDOF, 1);
  ...
  for (int i_task = 0; i_task < n_task; i_task++) {  //consider all tasks
    ...
    *jointVelocity = ((*jointVelocity)
        + P * invNonProjectedJ * ((*sot)[i_task].desired - (*sot)[i_task].jacobian * (*jointVelocity)));
...
void IKL::setJointsCapabilities(VectorXd limit_low, VectorXd limit_high, VectorXd maxVelocity,
...
void IKL::getTaskScalingFactor(Array<Scalar, Dynamic, 1> *a, Array<Scalar, Dynamic, 1> *b, MatrixD *W,
    Scalar *scalingFactor, int *mostCriticalJoint, Scalar maxScalingFactor) {
...
void IKL::getTaskScalingFactor_basic(Array<Scalar, Dynamic, 1> *a, Array<Scalar, Dynamic, 1> *b, Scalar *scalingFactor,
    int *mostCriticalJoint, Scalar maxScalingFactor) {
  Array<Scalar, Dynamic, 1> Smin, Smax;
  Scalar temp, smax, smin;
  Scalar inf = INF;
  int col;
...
void IKL::getTaskScalingFactor_fast(Array<Scalar, Dynamic, 1> *a, Array<Scalar, Dynamic, 1> *b, VectorXi *S,
    Scalar *scalingFactor, int *mostCriticalJoint, Scalar maxScalingFactor) {
...
Scalar IKL::SNSsingle(int priority, VectorD *higherPriorityJointVelocity, MatrixD *higherPriorityNull,
    MatrixD *jacobian, VectorD *task, VectorD *jointVelocity, MatrixD *nullSpaceProjector, double start_scale) {
...
Scalar IKL::OSNSsingle(int priority, VectorD *higherPriorityJointVelocity, MatrixD *higherPriorityNull,
    MatrixD *jacobian, VectorD *task, VectorD *jointVelocity, MatrixD *nullSpaceProjector, double start_scale) {
...
//----------------------------------------------------------------------- END W=I
  ...
  do {
    ...
    if (!isW_identity) {
      ...
      dotQopt[priority] = (*higherPriorityJointVelocity)
          + JPinverse * ((*task) - (*jacobian) * (*higherPriorityJointVelocity)) + tildeP * dotQn;
    ...
    if ((reachedSingularity) || (scalingFactor < 1e-12)) {
      ...
      if (bestScale >= 0.0) {
        W[priority] = bestW;
        dotQopt[priority] = (*higherPriorityJointVelocity)
            + bestInvJP * (bestScale * (*task) - (*jacobian) * (*higherPriorityJointVelocity)) + bestTildeP * bestDotQn;
      } else {
        //W[priority]=I;
        dotQopt[priority] = (*higherPriorityJointVelocity);
        /*
...
Scalar IKL::FSNSsingle(int priority, VectorD *higherPriorityJointVelocity, MatrixD *higherPriorityNull,
    MatrixD *jacobian, VectorD *task, VectorD *jointVelocity, MatrixD *nullSpaceProjector, double start_scale) {
...
Scalar IKL::FOSNSsingle(int priority, VectorD *higherPriorityJointVelocity, MatrixD *higherPriorityNull,
    MatrixD *jacobian, VectorD *task, VectorD *jointVelocity, MatrixD *nullSpaceProjector, double start_scale) {
...
Scalar QPsingle(int priority, VectorD *higherPriorityJointVelocity, MatrixD *higherPriorityAugmentedJacobian,
    MatrixD *jacobian, VectorD *task, VectorD *jointVelocity, MatrixD *augmentedJacobian, double start_scale) {
...
Scalar IKL::getJointVelocity_OSNS(VectorD *jointVelocity, StackOfTasks *sot, VectorD *jointConfiguration) {
  ...
  *jointVelocity = VectorD::Zero(robotDOF, 1);
  ...
  for (int i_task = 0; i_task < n_task; i_task++) {  //consider all tasks
    higherPriorityJointVelocity = *jointVelocity;
    higherPriorityNull = P;
    scaleFactors[i_task] = OSNSsingle(i_task, &higherPriorityJointVelocity, &higherPriorityNull,
        &((*sot)[i_task].jacobian), &((*sot)[i_task].desired), jointVelocity, &P);
    /*
...
Scalar IKL::getJointVelocity_FSNS(VectorD *jointVelocity, StackOfTasks *sot, VectorD *jointConfiguration) {
  ...
  *jointVelocity = VectorD::Zero(robotDOF, 1);
  ...
  for (int i_task = 0; i_task < n_task; i_task++) {  //consider all tasks
    higherPriorityJointVelocity = *jointVelocity;
    higherPriorityNull = P;
    scaleFactors[i_task] = FSNSsingle(i_task, &higherPriorityJointVelocity, &higherPriorityNull,
        &((*sot)[i_task].jacobian), &((*sot)[i_task].desired), jointVelocity, &P);
    if (scaleFactors[i_task] > 1)
      scaleFactors[i_task] = 1;
  }
...
Scalar IKL::getJointVelocity_FOSNS(VectorD *jointVelocity, StackOfTasks *sot, VectorD *jointConfiguration) {
  ...
  *jointVelocity = VectorD::Zero(robotDOF, 1);
  ...
  for (int i_task = 0; i_task < n_task; i_task++) {  //consider all tasks
    higherPriorityJointVelocity = *jointVelocity;
    higherPriorityNull = P;
    scaleFactors[i_task] = FOSNSsingle(i_task, &higherPriorityJointVelocity, &higherPriorityNull,
        &((*sot)[i_task].jacobian), &((*sot)[i_task].desired), jointVelocity, &PS);
