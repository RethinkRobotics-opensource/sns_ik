/*! \file IKL.hpp
 * \brief The Inverse Kinematic Library (\b IKL) header file
...
#ifdef _USE_DOUBLE_
  typedef Eigen::Matrix<double,Dynamic,Dynamic> MatrixD;/*! \typedef MatrixD 
							     dynamic sized matrix of \em float values (\em double if \b _USE_DOUBLE_ is defined)*/
  typedef Eigen::Matrix<double,Dynamic,1> VectorD;	/*! \typedef VectorD 
							     dynamic sized column vector of \em float values (\em double if \b _USE_DOUBLE_ is defined)*/
  typedef double Scalar;				/*! \typedef Scalar 
							     a \em float value (\em double if \b _USE_DOUBLE_ is defined)*/
  #define EPS 0.15
  #define LAMBDA_MAX 0.3
  #define EPSQ 1e-10
#else
  typedef Eigen::Matrix<float,Dynamic,Dynamic> MatrixD;/*! \typedef MatrixD 
							     dynamic sized matrix of \em float values (\em double if \b _USE_DOUBLE_ is defined)*/
  typedef Eigen::Matrix<float,Dynamic,1> VectorD;	/*! \typedef VectorD 
							     dynamic sized column vector of \em float values (\em double if \b _USE_DOUBLE_ is defined)*/
  typedef float Scalar;				/*! \typedef Scalar 
							     a \em float value (\em double if \b _USE_DOUBLE_ is defined)*/ 
  #define EPS 0.15
  #define LAMBDA_MAX 0.3
  #define EPSQ 1e-15
...
enum inv_solvers{STD, SCALE, SNS, OSNS, FSNS, OSNSsm, FOSNS, QP, CHIAVERINI, RP, STD_MIN_ACC, ACC,RP_ST};
...
typedef  vector<Task> StackOfTasks; /*! \typedef StackOfTasks
				     * the stack of desired tasks*/

/*
//! The actual state of robot joints
struct JointState{
  VectorD position; 		//!< actual joint configuration
  VectorD velocity;		//!< actual joint velocity
  VectorD acceleration;		//!< actual joint acceleration	
};
*/
...
class IKL{
  ...
  //vector<MatrixD> Jw;				//augmentations
  ...
  Scalar SNSsingle(int priority,VectorD *higherPriorityJointVelocity,MatrixD *higherPriorityNull, MatrixD *jacobian, VectorD *task, VectorD *jointVelocity,MatrixD *nullSpaceProjector,double start_scale=1);
  Scalar OSNSsingle(int priority,VectorD *higherPriorityJointVelocity,MatrixD *higherPriorityNull, MatrixD *jacobian, VectorD *task, VectorD *jointVelocity,MatrixD *nullSpaceProjector,double start_scale=1);
  Scalar FSNSsingle(int priority,VectorD *higherPriorityJointVelocity,MatrixD *higherPriorityNull, MatrixD *jacobian, VectorD *task, VectorD *jointVelocity,MatrixD *nullSpaceProjector,double start_scale=1);
  Scalar FOSNSsingle(int priority,VectorD *higherPriorityJointVelocity,MatrixD *higherPriorityNull, MatrixD *jacobian, VectorD *task, VectorD *jointVelocity,MatrixD *nullSpaceProjector,double start_scale=1);
  Scalar QPsingle(int priority,VectorD *higherPriorityJointVelocity,MatrixD *higherPriorityAugmentedJacobian, MatrixD *jacobian, VectorD *task, VectorD *jointVelocity,MatrixD *augmentedJacobian,double start_scale=1);
  bool pinv(MatrixD *A, MatrixD *invA, Scalar eps=EPS);   			//compute the pseudoinverse of A: it return 0 if A is (row) rank deficient
  bool pinv_P(MatrixD *A, MatrixD *invA, MatrixD *P, Scalar eps=EPS);   			//compute the pseudoinverse of A: it return 0 if A is (row) rank deficient / return also the null space projector P=(P-pinv(A)A)
  bool pinv_damped(MatrixD *A, MatrixD *invA,Scalar lambda_max=LAMBDA_MAX, Scalar eps=EPS);   			//compute the pseudoinverse of A: it return 0 if A is (row) rank deficient
  ...
  bool pinv_QR_Z(MatrixD *A, MatrixD *Z0, MatrixD *invA, MatrixD *Z, Scalar lambda_max=LAMBDA_MAX, Scalar eps=EPS);
  bool pinv_QR(MatrixD *A, MatrixD *invA, Scalar eps=EPS);
  void getTaskScalingFactor(Array<Scalar,Dynamic,1> *a, Array<Scalar,Dynamic,1> *b,MatrixD *W,Scalar *scalingFactor,int *mostCriticalJoint,Scalar maxScalingFactor=1.0);
  void getTaskScalingFactor_fast(Array<Scalar,Dynamic,1> *a, Array<Scalar,Dynamic,1> *b,VectorXi *S,Scalar *scalingFactor,int *mostCriticalJoint,Scalar maxScalingFactor=1.0);
  void getTaskScalingFactor_basic(Array<Scalar,Dynamic,1> *a, Array<Scalar,Dynamic,1> *b,Scalar *scalingFactor,int *mostCriticalJoint,Scalar maxScalingFactor=1.0);
...
public:
 ...
 void setSolver(inv_solvers solver=STD);//set the method used to compute the inverse kinematic
 void setSolver(const string solver);//set the method used to compute the inverse kinematic
 ...
 void setScaleMargin(double scale=0.9){scaleMargin=scale;}
 void setNullSpaceDampingFactor(double factor=0.95){nullSpaceDampingFactor=factor;}
 ...
 vector<Scalar> getTasksScaleFactor(){return scaleFactors;}
